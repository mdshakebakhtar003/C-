...
void leftboundary(Treenode*root){
    ...
    cout<<root->val<<" ";
    leftboundary(root->lc);
     if(root->lc==NULL) leftboundary(root->rc);
}
void bottomboundary(Treenode*root){
        if(root==NULL) return;
        if(root->lc==NULL && root->rc==NULL) cout<<root->val<<" ";
        bottomboundary(root->lc);
        bottomboundary(root->rc);

}
void rightboundary(Treenode* root){
    if(root==NULL) return;
    if(root->lc==NULL && root->rc==NULL) return;
      rightboundary(root->rc);
     if(root->rc==NULL) rightboundary(root->lc);
     cout<<root->val<<" ";
...
int main() {
   /* Treenode* a = new Treenode(1);
    ...
    c->lc = f;
    c->rc = g;*/
     int ar[]={1,2,3,4,5,INT_MIN,6,INT_MIN,INT_MIN,7,8,9};
